...
export const reducer = (state: IState = initialState, action: IAction) => {
	...
	switch (action.type) {
		case SET_DATA_AND_PARAMS: {
			...
			const { dependentQuestionsDict, dependentPagesDict } =
				getLogicalValidityCheckRulesByQuestionID(
		...
		case GO_TO_FIRST_DEVIATION_PAGE: {
			const { firstLocationWithDeviation, pageMovementLogs, pagesDict } = state;
			const { pageIndex } = firstLocationWithDeviation;
			const result = Object.values(pagesDict).find(
				(p: any) => p.order === pageIndex
			);
			const page = result?.page;
			const pageDocID = page!.docID;
			const index = (pageMovementLogs as number[]).indexOf(pageDocID);
			const newPageMovementLogs =
				index === -1 ? [] : pageMovementLogs.slice(0, index + 1);
			return {
				...state,
